# hover-disguise Specification

## Purpose
Provide a passive, hover-triggered disguise mode that covers TXT content with a boss-mode overlay by default, revealing the content only while the mouse cursor is inside the webview. Controlled via a native VS Code setting rather than a command.
