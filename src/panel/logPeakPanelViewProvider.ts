  ...
  private async openTxtFile(): Promise<void> {
    ...
    try {
      ...
      this.state = {
        ...
        fileName: vscode.workspace.asRelativePath(fileUri, false),
        content,
        displayFileName: getDisplayFileName(vscode.workspace.asRelativePath(fileUri, false)),
        metaText: formatLoadedMeta(vscode.workspace.asRelativePath(fileUri, false)),
        toolbarSourceLabel: getToolbarSourceLabel(),
        rowDecorations: createRowDecorations(content),
      };
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unable to read the selected text file.";
          ...
          <script>
            ...
            function renderMeta(state) {
              ...
              if (state.kind === "loading") {
                return "loading :: " + state.fileName;
              }

              if (state.kind === "error") {
                return "awaiting recoverable input";
              }

              return "awaiting local text source";
            }
            ...
            function renderPanelBody(state) {
              ...
              return \`
                <section class="loaded-state">
                  <div class="loaded-state__toolbar">
                    <span class="loaded-state__chip">\${escapeHtml(state.toolbarSourceLabel)}</span>
                    <button class="empty-state__action loaded-state__action" data-action="open-txt" type="button">Open TXT</button>
                  </div>
                  <div class="shell__rows">\${createLoadedRows(state.content, state.rowDecorations)}</div>
                </section>
              \`;
            }

            function render(state) {
              if (!(panelRoot instanceof HTMLElement) || !(shellMeta instanceof HTMLElement)) {
                return;
              }

              shellMeta.textContent = renderMeta(state);
              ...
              if (state.kind === "loaded") {
                ...
                if (rows instanceof HTMLElement) {
                  ...
                  const shouldRestore =
                    ...
                    persistedState.fileName === state.fileName;
                  if (shouldRestore) {
                    rows.scrollTop = persistedState.scrollTop;
                  }
                }
              }
            }
            ...
            document.addEventListener(
              ...
              (event) => {
                const target = event.target;
                if (!(target instanceof HTMLElement) || !target.classList.contains("shell__rows")) {
                  return;
                }
                ...
                scrollSaveTimer = window.setTimeout(() => {
                  ...
                  const nextState = {
                    ...
                    fileName: currentRenderedState.kind === "loaded" ? currentRenderedState.fileName : currentState.fileName,
                  };
